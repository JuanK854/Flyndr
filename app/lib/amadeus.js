...
const AIRPORT_SEARCH_URL = `${BASE_URL}/v1/reference-data/locations`;
...
async function getAccessToken() {
  ...
  if (!key || !secret) throw new Error('AMADEUS_CREDENTIALS_MISSING');
...
export async function searchAirportsAmadeus(keyword) {
  ...
  const res = await fetch(`${AIRPORT_SEARCH_URL}?${params}`, {
    headers: { Authorization: `Bearer ${token}` }
  });

  if (!res.ok) throw new Error('AMADEUS_AIRPORT_SEARCH_FAILED');
